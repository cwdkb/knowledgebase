...
(function () {
  ...
  // If the browser restores this page from back/forward cache (e.g. hitting Back, or an
  // already-open tab regaining focus after signing out elsewhere), no script re-runs by
  // default — the old "visible, logged in" state would just sit there. Force a real
  // reload so the check above runs again against current session state.
